...
- 2026-06-22: [df] Soạn + chuẩn hóa description cho dataflow (tiếng Việt, mô tả fact, ghi rõ output dataset). Ghi LIVE lên OAC `definition.description` cho 30/30 dataflow hiện hành (verify steps/links/vị trí không đổi sau mỗi PUT). Thêm `description:` cho 28 entry tương ứng trong `dataflow_catalog.yaml`. LƯU Ý: live hiện 30 dataflow (+2 sequence); catalog còn 40 entry (12 flow đã xóa trên OAC + thiếu 2 flow mới `(KGR) 5. DTF_CALC_MIS Ver 2`, `KGR_DF_Nganh_Metrics_v3_bk_2026-06-22`) → cần re-extract+rebuild để đồng bộ đủ.
