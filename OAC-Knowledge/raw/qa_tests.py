...
# FLOOR-based (refresh-proof): bắt drop thảm hoạ, KHÔNG đỏ giả khi refresh hợp lệ đổi count. Integrity đầy đủ ở validate_kb.
check("I3","Counts floor: 4 wb / >=60 ds / >=32 df / >=55 phys", len(wb)==4 and len(ds)>=60 and len(dfc)>=32 and len(phys)>=55, f"{len(wb)}/{len(ds)}/{len(dfc)}/{len(phys)}")
