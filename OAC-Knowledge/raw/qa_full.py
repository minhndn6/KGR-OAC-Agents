...
try:
    ...
    # FLOOR-based (refresh-proof): bắt DROP THẢM HOẠ (build bug rớt nửa catalog), KHÔNG đỏ giả vì refresh hợp lệ đổi count.
    # Integrity đầy đủ (bidirectional/orphan) đã ở 1.1/1.2. Floor đặt dưới mức hiện tại (67/36/60/4).
    t("S9","9.6b_counts_floor", len(DS)>=60 and len(DF)>=32 and len(PH)>=55 and len(WB)==4, f"{len(DS)}/{len(DF)}/{len(PH)}/{len(WB)} (floor 60/32/55/4)")
