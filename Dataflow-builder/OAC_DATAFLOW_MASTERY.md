...
- **POST (TẠO MỚI — CREATE = POST, KHÔNG phải PUT):** `POST /ui/dv/ui/api/v1/dataflows?folderPath=<urlenc /@Catalog/users/EMAIL>&dataFlowName=<urlenc NAME>` body = DataGenAttributes `{"datagen-name":NAME,"display-name":NAME,"dataflow-name":NAME,"datagen-type":"DATAFLOW","definition":<def>}` → `{dataflowId:"'<server-guid>'.'NAME'", success:true, requestStatus:201}`.
  ...
  - ⚠️⚠️⚠️ **FLOW POST-CREATE MANG NAMESPACE GUID, KHÔNG phải `'email'.'NAME'` (verified 2026-07-16, KGR_DF_DAILY_NGANH_LITE_v1).** POST trả `dataflowId:"'<server-guid>'.'NAME'"` (vd `'d92635e8-…'.'KGR_DF_…'`). **Mọi GET def / deep-link editor / PUT / run PHẢI dùng đúng id GUID này** — dùng `'minhndn@bizin.vn'.'NAME'` (dạng email như flow tạo-bằng-UI) → **GET 404 `DSS 40400 DataGenerator not found`**, và mở editor bằng artifactid email → **canvas TRỐNG + console `Cannot read properties of undefined (reading 'steps')`** (report.js `_onLoadSuccess`) — DỄ TƯỞNG NHẦM là def hỏng/§11.5, nhưng def hoàn toàn OK, chỉ SAI id. Lấy id đúng từ: (a) `dataflowId` trong response POST, hoặc (b) catalog `GET /api/v2/items?...&search=<name>` → `childItems[].id`. Deep-link editor: `…project.jsp?artifactid=<urlenc "'<guid>'.'NAME'">&pageid=visualAnalyzer&reportmode=dataflow`. (Flow tạo bằng UI thì namespace = email nên deep-link email chạy — chỉ REST-POST mới ra GUID.)
