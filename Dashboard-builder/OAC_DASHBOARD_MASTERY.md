...
- **Workbook chính (PRODUCTION — ADD-only, KHÔNG nghịch):** `(KGR) DB01.Revenue_v1.1` tại `/@Catalog/shared/(KGR) Report/` (⚠️ ĐÃ DỜI — verified live 2026-07-31 qua `items?path=`; path cũ `/@Catalog/shared/(KGR) 1.Implement/` giờ 404 OBI-01401, vị trí đó nay là `(KGR) DB01.Revenue_v1.3`. Luôn xác nhận path bằng `GET items` trước khi ghi, đừng tin path cũ trong file này).
...
- URL DB01 (path mới, verified 2026-07-31): `…?pageid=visualAnalyzer&reportmode=full&reportpath=%2F%40Catalog%2Fshared%2F(KGR)%20Report%2F(KGR)%20DB01.Revenue_v1.1&viewermode=false`
...
- ⭐⭐ **TẠO CALC HÀNG LOẠT + SET NUMBER FORMAT bằng model-save (verified live 2026-07-27, rẻ hơn UI hàng chục lần):**
  - **Calc**: thêm vào `criteria.columns.children[]`: `{"columnID":"<ID>","userExpression":true,"type":"saw:regularColumn","columnFormula":{"expr":{"children":[],"type":"sawx:sqlExpression","expression":"<Logical SQL>"}},"columnHeading":{"caption":{"text":"<tên hiện>"}},"folderID":"<id folder>"}`; folder mới = push `{"folderID","folderName":{"caption":{"text"}},"children":[],"sort":""}` vào `folders.calculations.children[0].children`. Tham chiếu calc↔calc = **`@calculation("<columnID>")`**. POST → **reload deep-link** → calc hiện trong My Calculations, dùng được ngay (đã verify 21 calc gồm CASE/COALESCE/RANK…BY/ABS).
  - **Number format per-measure**: `views.children[viz].viewConfig.settings["viz:chart"]["numberFormat<columnID>"] = {"style":"percent"|"decimal","currency":"USD","useGrouping":true,"maximumFractionDigits":N,"useAbbreviation":true|false,"abbreviationScale":"b"|"m","negativeValuesStyle":"default","bIsNested":true}`. Persist qua reload ✅ (viz đời cũ dùng key `bidvtchart_number_format_<colID>` — cùng payload).
  - Đọc số THẬT để cross-foot: `data/poll` (không chỉ `executeOrPoll`) cũng chứa response `report.views[...]` — lấy request **cuối** trong list_network_requests.
- ⭐⭐ **NẠP PATCH LỚN VÀO TRANG BẰNG FILE-INPUT BRIDGE (verified 2026-07-27, thay cho fetch localhost):** `evaluate_script` với payload model 100KB+ inline → **`Runtime.callFunctionOn timed out`**; fetch `http://127.0.0.1:3000/...` từ trang OAC **bị CSP `connect-src 'self'` chặn** (abort câm, không lỗi rõ). ⇒ Cách chạy: (1) tạo 1 lần `<input type=file id=claudeBridge>` gắn vào body + hàm `window.__applyPatch(patch, dryRun)` (GET model → mutate → POST); (2) `take_snapshot` lấy uid của input; (3) **`upload_file(uid, "<đường dẫn patch .json>")`**; (4) `evaluate_script`: `await document.getElementById('claudeBridge').files[0].text()` → JSON.parse → gọi applier. ⚠️ Reload trang là mất cả input lẫn applier → cài lại (2 call). Patch nên là **DIFF** (`newViews/replaceViews/removeViews/newLayouts/layoutPatch/newFCC/fccAddControls/colorMap`), không phải cả model — applier GET model tươi nên không đè mất thay đổi song song.
- ⭐⭐⭐ **SORT trên viz: `columnSort` trong logicalDataModel KHÔNG sort gì cả — sort THẬT là `columnOrder` (verified 2026-07-27, sau khi 21 viz dựng bằng model-save đều ra thứ tự A→Z dù LDM đã ghi `direction:descending`):**
  - **CHART**: `views.children[viz].nestedViews.children[0].view.dataModels.children[0].edges.children[0]` (axis row) thêm:
    `"columnOrder":{"children":[{"columnID":"<DIM>","direction":"descending","QDR":{"children":[{"specialDimension":"grandTotal","members":{"children":[{"text":"gt_column"}],"type":"saw:stringMembers"}},{"specialDimension":"measure","members":{"children":[{"text":"<MEASURE_COLID>"}],"type":"saw:specialValueMembers"}}]}}]}` — QDR chính là "sắp theo measure nào".
  - **TABLE**: `views.children[viz].dataModels.children[0].edges.children[0].columnOrder = {"children":[{"columnID":"<c1>","direction":"ascending"},{"columnID":"<c2>","direction":"descending"}]}` (KHÔNG cần QDR; nhiều khoá = sắp nhiều cấp, đúng thứ tự mảng).
  - Nên ghi **cả hai**: `columnOrder` (engine dùng) + `columnSort` trong LDM (UI panel/dialog "Sort Order" đọc để hiển thị). Chỉ có LDM = trông như đã sort trong dialog nhưng render sai.
  - **Sắp theo THÁNG** khi category là `PERIODNAME` (chuỗi 'May 2026'/'Jun 2026'): dùng `columnOrder` **descending KHÔNG QDR** → May trước Jun (đúng lịch); ascending cho ra Jun trước May (sai lịch).
- ⭐⭐ **FILTER MỨC VIZ + TOP-N + NGƯỠNG bằng model-save (verified 2026-07-27):** thêm collection `{"name":"view!N","filterControls":{"children":[...]}}` vào `filterControlCollections.children` **và** set `filterControlCollectionName:"view!N"` trên leaf tương ứng trong `layouts`. (Collection tên `canvas!N` = filter cả canvas.)
  - **Top-N theo measure**: `filterModelClassName:"obitech-topbottomfilter/topbottomfilter.TopBottomDimFilterModel"`, `simplifiedDataElementFormula:<expr dim>`, `topBottomDimFilterFactColumnID/Formula:<measure>`, `filterOperator:{"op":"top"}`, `formula.expr.expression = "aggregate(<measure expr> by <dim expr>)"`, `filterControlDefaultValues.children[0].text = "<N>"`, `filterByColumns.children=[{expression:<dim expr>,type:"sawx:sqlExpression"}]`. ✅ Chạy đúng cả khi dim là **calc COALESCE** (nhãn ghép 2 phía) trên workbook blend.
  - **Ngưỡng số**: cùng khung, `filterModelClassName:"obitech-rangefilter/rangefilter.RangeFilterModel"`, `filterOperator:{"op":"greaterOrEqual"}`, default value = số dạng chuỗi, formula = `aggregate(<measure> by <grain cần chặn>)` → lọc SAU aggregate (vd DT/SKU ≥ 100.000.000).
  - **Loại NULL / loại danh sách**: `filterOperator:{"op":"notIn"}` + `filterControlConfig.settings.isExclusive:true`; NULL = sentinel **`*)nqgtn(*`** (kèm `caption:{text:"<NULL>"}`). Loại "linh kiện" = liệt kê ĐỦ giá trị (44 bên TT, 75 bên SFC — lấy live bằng `GROUP BY`), OAC không có LIKE trong list-filter.
- ✅ **BẰNG CHỨNG BỔ SUNG cho escape-hatch row-calc (2026-07-27, canvas Top SKU):** blend + cột thật + row-calc `KGR_M1/M2` giữ đúng số tới **grain SKU** — cross-check mù bằng `queryData` 25+ điểm (tổng công ty, tháng, ngành 2 phía, chuỗi, khách hàng, nhóm, 3 SKU) **lệch 0**. Bảng blend 10 cột (dim + 3 SL + 2 DT + 2 GP) vẫn đúng ⇒ wall chỉ đánh vào calc-AGGREGATE, không đánh vào số cột thật/row-calc dù viz phức tạp.
- 📌 **SFC dataset (2026-07-27):** hiện trong data panel là **"(KGR) DW_SFC"** nhưng tên XSA thật là `'(KGR) SFC Dataset (có AI)'`; bảng fact LIVE = `"DW_NS_X_SFC_LINES_CF_1"` (cột `Số lượng`/`DT SFC`/`Lãi gộp`/`CP CKKM`) — bảng `DW_NS_X_SFC_LINES_CF` (không `_1`) là bản CŨ còn sót trong calc đời trước.
  - **Đã remap 27/07 (GATE-5 rework):** `CUSTCOL_SCV_QUANTITY_SFC` → CF_1."Số lượng"; `DSSFC` → Σ CF_1."DS W1..W5" (tên `DS W5` sống NGUYÊN trên CF_1 — méo phông chỉ ở describe); FCC canvas!13 remap cùng kiểu. Verify: tile canvas!3 SL 645.052 / DS 400.709.025.500 = đúng Jul 2026 query CF_1 độc lập.
  - ⚠️ **CÒN 18 ref chết TIỀN SỬ** trong criteria trỏ bảng CŨ (HngBn, DoanhthuSFC, UnitCost_2, Givn_15, LNGp_2, CKKM, GPRng, CPKM, VAT_2, DSW1-5, SLW1-5 — đều của bảng view!157 canvas!3): mở canvas!3 vẫn bật dialog Error-Loading-Data liệt kê đúng 18 tên này. **Bấm OK vô hại NHƯNG UI-save sau đó sẽ prune** (wall §14). Save qua POST projects/json (GET tươi từ server) MIỄN NHIỄM với dialog. Hết hẳn dialog = remap nốt 18 ref (việc riêng, cần owner duyệt vì dính cột %/AVG/MAX ngữ nghĩa đời cũ).
- ⚠️ **Trước khi spawn builder: kiểm cookie/profile còn sống** (mở deep-link, nếu rơi về IDCS signin thì DỪNG báo owner) — lượt 2026-07-27 đã mất 1 lượt agent vì cookie chết.
...
| ⭐ Deep-link `project.jsp` mở workbook ở **VIEW mode** (không có radio Data/Visualize/Present, chỉ có nút "Edit") | click nút **"Edit"** rồi mới thấy 3 mode; hoặc dùng `home.jsp…&viewermode=false` (§2). Verified 2026-07-27 |
| ⭐⭐ Dialog **"Error Loading Data"** (cột dataset đã bị xoá, vd `SalesCategory`) — bấm OK rồi Save thì OAC **XOÁ cột khỏi criteria + khỏi viz canvas CŨ** → **vi phạm ADD-only âm thầm** | Trước khi build: clone backup; sau khi build xong: **splice khôi phục** entry `criteria.columns` + nguyên node `views.children[view!N]` từ model backup rồi POST projects/json (§13). Verified 2026-07-27 trên view!159 |
| Blend Data: chọn cột trong picker bằng script/jQuery = no-op | **REAL CDP click** trên treeitem (như chip/dropdown §1). Mở dialog = **double-click badge số** trên connector Data Diagram |
| Blend Data: list Available Values **tự sắp xếp lại sau MỖI dblclick** → dblclick tiếp trúng dòng khác | thêm **từng giá trị một**, verify "Selections (n/N)" + đọc lại Selected Values List sau mỗi lần; thừa thì dblclick trong Selections để bỏ |
| ⭐⭐⭐ **CALC (My Calculations) làm MEASURE + GROUP theo chiều trên viz BLEND → trả SỐ CẤP TỔNG lặp lại mọi hàng** (verified live 2026-07-27: scorecard theo Ngành hiện %đạt 71,2% / %GP 37,1% / Δ −14,8% GIỐNG HỆT cả 5 ngành; DT phồng 1,7–2,4×) | Bản mở rộng của wall §10 (FILTER cross-group). **Measure là CỘT THẬT (`Doanh thu thực tế`, `DT SFC`) group ĐÚNG** (Monthly Run theo PERIODNAME khớp §6 tuyệt đối); **calc thì KHÔNG**. ⇒ Viz blend theo chiều: chỉ dùng cột thật; tỷ lệ/chênh lệch để ở **KPI/tile mức TỔNG** (calc chạy đúng ở đó) hoặc dựng cột thật bằng DATAFLOW. **Cross-foot NGAY viz đầu tiên có group-by** |
| Calc COALESCE (nhãn ghép 2 phía) làm **Category** viz blend | Số vỡ → dùng cột chiều của MỘT phía (thường TT); member chỉ-có-SFC dồn vào hàng trống — ghi rõ trong note (fallback F5) |
| Đổi columnID hàng loạt bằng string-replace trên model | ⚠️ Replace chạm cả **định nghĩa cột trong `criteria.columns`** → trùng ID, workbook báo "items could not be found". Chỉ replace trong `views`/`layouts`/`filterControlCollections`, **chừa `criteria`** |
| Xoá canvas bằng model-save → workbook TRẮNG, console `currentView attribute is set to a view that is not a canvas` | Set lại `model.views.currentView` = **INDEX (số)** của canvas còn sống trong `views.children` |
| POST model có `annotations` sai kiểu → workbook TRẮNG, console `Failed to validate project criteria … {"annotations":{"type":"null"}}` | **Bỏ hẳn key `annotations`** khi không có note (đừng để null/rỗng); note tạo qua UI "Add Note" |
| Cột criteria KHÔNG viz nào dùng bị OAC **prune khi Save** (vd `DTSFC` biến mất sau khi bỏ chip) | Trước khi model-save tham chiếu 1 columnID, **verify nó còn trong `criteria.columns`**; thiếu thì thêm lại entry |
| ⭐⭐⭐ **Workbook ĐÃ BLEND: KHÔNG tồn tại "viz đơn-dataset" thật** (B0 verified 2026-07-27 #2: bar Category=cột TT + measure=calc chỉ-tham-chiếu-cột-TT vẫn trả HẰNG SỐ 0,2354…/0,2101… cho MỌI ngành; member một-phía 'Khác'/Chung RỚT khỏi trục) | Mọi viz đều đi qua blend data model → tách "viz đơn" KHÔNG phải lối thoát cho calc-aggregate. Cùng viz đó, blend + CỘT THẬT vẫn đúng tuyệt đối |
| ⭐⭐⭐ **ESCAPE HATCH cho wall calc×blend×group: calc ROW-LEVEL (KHÔNG SUM/aggregate trong biểu thức, vd `"LN Gộp" - "CP CKKM"`) group ĐÚNG như cột thật trên viz blend** (B0x verified 2026-07-27: GP ròng TIỀN 2 phía theo Ngành khớp derived-expected từng ngành — Water 77B/42B, Home 25,5B/47B, Cold 7,1B/−8,2B, member một-phía giữ nguyên; B0r: phân hoá đúng tới grain SKU) | "Tổng-trừ-tổng" → viết thành calc row-level, để default agg SUM. **RATIO (Σ/Σ) theo chiều KHÔNG cứu được** (buộc aggregate trong công thức) → ratio chỉ ở KPI cấp tổng, hoặc dựng cột thật bằng dataflow |
| ⭐⭐⭐ **`COUNT(DISTINCT <cột>)` trên workbook BLEND trả số của TẬP JOIN, không phải của dataset đó — kể cả ở KPI tile cấp tổng** (verified 2026-07-27: tile hiện 37 khách TT / 37 khách SFC, trong khi query độc lập cho 491 và 45; hai phía ra số GIỐNG NHAU = dấu hiệu nhận biết) | Bổ sung cho wall calc-aggregate: KPI tổng chỉ an toàn với **SUM/tỷ số của SUM**; **COUNT DISTINCT thì KHÔNG**. Cần đếm member → query ngoài (queryData) rồi đưa vào note, hoặc dựng cột đếm sẵn bằng dataflow. ⚠️ Hai measure "đếm" bằng nhau chan chát ở hai nguồn khác hẳn nhau = nghi ngay, đừng ship |
| ⭐⭐⭐ **Sort đặt bằng `columnSort` (logicalDataModel) KHÔNG ăn** — viz render theo thứ tự A→Z dù model ghi `descending`; dialog "Sort Order" của UI lại hiện đúng ⇒ false-positive khi verify bằng dialog | Sort thật = **`columnOrder`**: chart đặt trong `nestedViews…edges.children[0]` kèm **QDR** (grandTotal + measure); table đặt trong `dataModels.children[0].edges.children[0]` (không QDR). Ghi CẢ hai (columnOrder + columnSort). Category `PERIODNAME` → `descending` không QDR mới ra May→Jun (§13) |
| ⭐⭐ **comboMultiLayerChart (Overlay) dựng bằng model-save KHÔNG nhận layer LINE** — `dataLayersInfo {ndm1:line-dashed, ndm2:bar}` + measures chia đúng layer, grammar panel hiện đúng "Line/Bar" đúng chip, nhưng render vẫn 2 BAR ("2 Bars"); 2 lần chỉnh nested đều thua; chip menu Overlay không có mục Bar/Line (chỉ Y2/Sort/Filter/Delete) | Bar+line qua model-save = **`oracle.bi.tech.chart.combo`** (verified 2026-07-27): khung y hệt chart.bar + `viz:barlineareachart.measureInfos={"<barCol>":{"type":"bar"},"<lineCol>":{"assignedToY2":"off"}}` (không khai type = LINE) + `viz:chart.measures={"<lineCol>":{"lineStyle":"dashed"}}` + `viz:chart.styleDefaults={"markerDisplayed":"on"}` → cột + đường nét đứt chấm tròn. Line cần màu riêng mà không đụng màu workbook của cột gốc → calc **identity row-level** (vd `KGR_L1 = SFC."DT SFC"`) + colorMap riêng (#F16522) |
| ⭐⭐⭐ **Canvas-filter (pill cột) trên workbook BLEND lan qua match-columns — KHÔNG dataset-scoped** (verified 2026-07-27 canvas Tương tác: chỉ pill TT `Tên Ngành='Water care'` mà tile `DT SFC` rút 550,4→**321,9 tỷ = đúng tổng SFC Water care**) | Hệ quả thuận: 1 pill lọc đúng CẢ 2 phía cho giá trị có mặt 2 nguồn; **cặp pill cùng giá trị (AND) không rơi dòng** (2 pill Water care vẫn 416,5/321,9 tỷ khớp query từng phía; cặp Nhóm RO khớp 369,6/286,7 tỷ + GP 2 phía). Giá trị chỉ-một-phía (NAME='Chung'): lọc bên có → measure bên kia về NULL, đúng nghĩa. Note "chọn cùng giá trị 2 ô" giữ làm hàng rào. Supersede cách đọc "pill dataset nào chỉ áp viz dataset đó" |
| Pill để trống (All) qua model-save: columnFilterControl op `in` KHÔNG kèm `filterControlDefaultValues`/`filterControlSource` → hiện "0 selected", không lọc (verified) | ⚠️ `label.caption.text` trên columnFilterControl KHÔNG đổi tên pill hiển thị (vẫn tên cột) — muốn pill tên tùy chỉnh chưa có đường model-save; phân biệt 2 nguồn bằng chính tên cột (Tên Ngành vs NAME, Nhóm SP vs Nhóm sản phẩm…) |
| ⚠️ **fetch tới `http://127.0.0.1:<port>` từ trang OAC bị CSP `connect-src 'self'` chặn** (abort câm sau timeout) + `evaluate_script` payload >100KB → `Runtime.callFunctionOn timed out` | Nạp patch bằng **file-input bridge**: `<input type=file>` + `upload_file(uid, path)` + `files[0].text()` (§13) |
| ⚠️ Reload trang xoá sạch `window.__applyPatch` và input bridge đã cài | Sau MỖI `navigate_page(reload)` phải cài lại applier + input rồi mới snapshot lấy uid |
| ⭐⭐ **RANK() calc vỡ trên workbook blend BẤT KỂ đầu vào** (B0r verified 2026-07-27: bảng Nhóm SP×Mã hàng — C10a %GP/SKU = hằng 0,17; RANK(C10a BY nhóm) = 1 đồng loạt; **RANK(SUM(row-expr) BY nhóm) cũng = 1 đồng loạt** dù chính SUM(row-expr) group đúng khi làm measure thường) | RANK là analytic → cùng số phận aggregate-calc trên blend. Top-N: dùng **TopN filter/sort của viz theo measure row-calc/cột thật** (cơ chế filter, không phải calc), hoặc dataflow dựng sẵn cột rank/%GP |
| ⭐⭐⭐ **KPI TILE 1-measure chạm 1 dataset trên workbook BLEND trả SỐ SAI** (verified live 2026-07-28 canvas!26: tile `Doanh thu thực tế` hiện 303,1 tỷ thay vì 665,1 tỷ = subset inner-join; tile calc %GP-một-phía hiện HẰNG 23,5%/21,0%). Tile chạm **CẢ HAI** dataset (C6 = ΣDT_TT/ΣDT_SFC, C8) đúng tuyệt đối — vì query kéo cả 2 nhánh blend | **Ép phía kia vào query, toán học không đổi**: tiền → row-calc `IFNULL(A,0) + 0 * IFNULL(B,0)`; ratio → `(SUM(x) − SUM(y) + 0*SUM(IFNULL(B,0))) / SUM(z)`. Sau fix: 665,1 tỷ / 550,4 tỷ / 16,9% / 14,6% khớp §6 tuyệt đối, và khớp cả khi lọc pill (Water care 416,5/321,9/18,5%/13,1%). Bài học: tile 1 nguồn trên workbook blend = **luôn nghi**, cross-foot ngay |
| ⭐⭐⭐ **CF qua model-save — SỬA LẠI 2026-07-28 #2 (supersede "đặt cfrule lên layer của chính cột"): mọi `cfrule.*` phải nằm trên LAYER CUỐI CÙNG của row-edge, KHÔNG phải layer của cột được tô.** Bằng chứng: 2 rule model-born cfrule.KGR_M1/M2 đặt đúng "layer của chính cột" (layer giữa) vẫn render trong suốt `rgba(255,255,255,0)`; rule KGR_M4 "chạy được" chỉ vì KGR_M4 TÌNH CỜ là layer cuối. Xoá 2 rule chết, tạo lại bằng UI dialog rồi diff model: UI dồn CẢ 3 `cfrule.*` (kèm luôn `dataBarMin/Max.*` từ layer[0]) về **một chỗ duy nhất = layer cuối row-edge** → render chuẩn. Layer giữa = vùng chết engine không đọc | Công thức đủ 3 mảnh cho TABLE: (1) rule trong `reportConfig.settings.conditionalFormatRules.settings.rules`; (2) `views[v].viewConfig.settings["viz:common"].conditionalFormatting.activeRules=[{ruleID,generationID,field:["columns"]}]` — `generationID` phải KHỚP rule (trùng genID giữa 2 rule vẫn chạy, không phải thủ phạm); (3) TOÀN BỘ `propertyAdditions.children += {"id":"cfrule.<colID>","valueColumnID":"<colID>","aggRule":"default","stacked":false,"placement":"all","grainEdge":"none","acrossMeasures":"single"}` gắn vào **layer CUỐI của `edges.children[0].edgeLayers.children`**. Ghi chú fidelity theo UI: rule 1-điều-kiện UI ghi `type:"target"` (nhiều fragment nối `parentId` mới là `"threshold"`), hex thường (`#f1a884`) — không phải điều kiện render. Đã verify nhân bản bằng model-save đúng công thức này sang view!500/502/512 → màu lên cả 3. **Data bars**: `viz:grid.dataBars.columns[<colID>]={type:"bar",color:{...}}` + `dataBarMin/Max.<colID>` (`placement:"first_cell"`) — nằm layer[0] hay layer cuối đều từng render, UI chuẩn hoá về layer cuối. TILE thì CF vẫn KHÔNG ăn (tile không có edgeLayer để gắn `cfrule.*`) → fallback màu tĩnh, không tô tĩnh cho %đạt/Δpp |
| ⚠️ **POST projects/json ngay sau khi trang reload (hoặc sau 1 lần Save bằng UI) trả `{"retryRequestAfterSessionRestore": true, "data": "CSRF mismatch requires retry"}`** dù token vẫn đúng (đối chiếu `GET /ui/dv/ui/api/v1/sessioninfo/ext?stateId=4` → field `csrftoken` KHÔNG đổi) | Đúng như tên: **POST lại y hệt lần 2 là 200 `success:true`**. Đừng đi săn token mới, đừng đổi payload. NHƯNG nếu retry lần 2 vẫn mismatch → token ĐÃ xoay thật (thường do owner mở phiên UI mới) → GET sessioninfo lấy `csrftoken` mới rồi POST lại |
| ⭐⭐⭐ **OWNER MỞ PHIÊN UI SONG SONG = model-save của agent BỊ ĐÈ TOÀN PHẦN khi owner bấm Save** (2026-07-28 ~11:58: 3 POST thành công của agent (CF + gọn pill + dọn drill) bị save UI của owner — load model từ TRƯỚC các POST — xoá sạch; phiên UI giữ NGUYÊN model trong RAM, không merge). Kèm dấu hiệu: csrftoken xoay liên tục, tab agent bị đóng, `lastUpdatedTime` nhảy không do mình | **Trước MỖI POST: so `lastUpdatedTime` với mốc save cuối CỦA MÌNH** — lệch lạ = owner đang sống → DỪNG GHI, chỉ đọc, báo cáo; patch giữ dạng script re-apply được vài phút sau khi owner đóng phiên. Giờ hành chính mặc định coi như owner online |
...
- **Number format**: MONEY = M (DP0, Abbreviate M); QUANTITY/volume = units (KHÔNG M); RATIO = Percent (DP0).
  - 📌 **NGOẠI LỆ workbook-level đã chốt (2026-07-27, `(KGR) BRD.BC03-04-05_Compare SFC_Actual`):** tiền = **tỷ (B), 1 lẻ**; ratio = **% 1 lẻ**; số lượng = units 0 lẻ. Lý do: số ở đây hàng trăm tỷ VND, để M/DP0 thành dãy 6 chữ số khó đọc. Reviewer/builder phiên sau **đừng "sửa lại" về M** — nhưng cũng đừng lan ngoại lệ này sang DB01/DB02 (đang dùng M): so cạnh nhau sẽ lệch tông.
