...
| **Temp/scratch** (snapshot, dump thử, ảnh, model tạm, network response) | **`<project>/_work/`** | gitignored |
| **Tài liệu handoff/review/brief liên-project** (không thuộc 1 repo nào) | **`C:\Project\_work\review\`** (vùng temp cấp-workspace) | ngoài repo |
| **Staging pipeline** (raw JSON dump + script build để re-distill) | **`OAC-Knowledge/_work/staging/`** | gitignored |
| **Provenance tối thiểu** (raw cần để rebuild, có thể giữ) | `OAC-Knowledge/raw/` (json gitignored, .py tracked) | mixed |
| **Secrets** | `C:\Project\KGR-OAC-Agents\.secrets\` | gitignored (mọi repo) |
| **Archive (reversible)** | `C:\Project\_archived\` + `_RESTORE_MANIFEST.md` | ngoài repo |
| **Learning log** (tích lũy tri thức) | `OAC-Knowledge/learnings/` | TRACKED (xem LEARNING.md) |
...
| `KGR_LOCK_DIR` | Thư mục write-lock orchestrator | `<WS>/_orchestration/locks` |
